...
public static class DbSeederGenerator
{
    public static string Generate(AssignmentSeedData seed, string signature, string rootNamespace = ProjectTargetHelper.ReferenceRootNamespace)
    {
        ...
        if (seed.OrdersEnabled)
        {
            ...
            sb.AppendLine("            var today = DateTime.UtcNow.Date;");
            sb.AppendLine("            var orders = new[]");
            sb.AppendLine("            {");
            sb.AppendLine("                new Order");
            sb.AppendLine("                {");
            sb.AppendLine("                    Article = \"ORD-1001\",");
            sb.AppendLine("                    Status = OrderStatus.New,");
            sb.AppendLine("                    OrderDate = today,");
            ...
            sb.AppendLine("                new Order");
            sb.AppendLine("                {");
            sb.AppendLine("                    Article = \"ORD-1002\",");
            sb.AppendLine("                    Status = OrderStatus.New,");
            sb.AppendLine("                    OrderDate = today.AddDays(-1),");
            sb.AppendLine("                    DeliveryDate = today.AddDays(2),");
            sb.AppendLine("                    PickupCode = \"739154\",");
            sb.AppendLine("                    PickupPointId = pickupPoints[Math.Min(1, pickupPoints.Length - 1)].Id,");
            sb.AppendLine("                    ClientId = client?.Id");
            sb.AppendLine("                },");
            sb.AppendLine("                new Order");
            sb.AppendLine("                {");
            sb.AppendLine("                    Article = \"ORD-1003\",");
            sb.AppendLine("                    Status = OrderStatus.Completed,");
            sb.AppendLine("                    OrderDate = today.AddDays(-4),");
            sb.AppendLine("                    DeliveryDate = today.AddDays(-1),");
            sb.AppendLine("                    PickupCode = \"165802\",");
            sb.AppendLine("                    PickupPointId = pickupPoints[0].Id,");
            sb.AppendLine("                    ClientId = client?.Id");
            sb.AppendLine("                }");
            sb.AppendLine("            };");
            sb.AppendLine("            await db.Orders.AddRangeAsync(orders);");
            sb.AppendLine("            await db.SaveChangesAsync();");
            sb.AppendLine("        }");
        }
